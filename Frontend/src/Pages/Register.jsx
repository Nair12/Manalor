...
const Register = () => {
 ...
 const SubmitHandler = async () => {
  ...
  if(RegisterUser.fulfilled.match(result)){
   ...
   navigate("/EmailConfirm",{state:{email:email}})
  ...
  return (
    <StyledWrapper>
      <form className="form" onSubmit={(e)=>{
            e.preventDefault()
            SubmitHandler()}} >
        <div className="title">Hello,<br /><span>Register for continue</span></div>
        ...
        <h3 name ="avatar">Avatar</h3>
        <FileInput SetImage = {setAvatar}/>
  ...
  <input type="checkbox" required style={{ width: '20px', height: '20px' }} />
    ...
    <a href="/Policy" target="_blank" rel="noopener noreferrer">our user agreement</a>
  </span>
   </div>


        <button className="button-confirm" type="submit">Let`s go →</button>
        <Link to={"/Login"}>Already have account? Sign up here!</Link>
      </form>
    </StyledWrapper>
